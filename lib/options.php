...
class Esdc_options{
    ...
    function create_options_page() {
        add_menu_page('Download Counter', 'Download Counter', 'administrator', __FILE__, array(&$this,'options_page') );
    }
    ...
    function populate_stats($from="", $to="", $file="",$limit=""){
        $db = new Esdc_db();
    	$results = $db->get_data($from, $to, $file, $limit); ?>
      ...
      <div id="theme-options-wrap">
        ...
        <div class="esdc-search esdc-container">
        	...
        	<div id="esdc-search-results">
        		<?php $this->populate_stats('','','',10); ?>
        	</div>
        </div>
    	<div class="esdc-options esdc-container"><?php $this->options(); ?></div>
    ...
    function register_and_build_options(){
      ...
      add_settings_field('esdc_file_types','File Types: ',array(&$this,'file_types'),__FILE__,'main_section');  
    }
    
    function validate_file_types($option){
    	$option = explode(',',$option);
    	array_map(array(&$this,'remove_whitespaces'),$option);
        //put any validation on the option here.
        return $option;
    }
    
    function remove_whitespaces($var){
